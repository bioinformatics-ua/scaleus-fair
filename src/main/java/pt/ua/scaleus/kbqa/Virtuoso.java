...
public class Virtuoso {
	...
	public void close() {
		try {
			...
			if (this.useCache) {
				...
				FileUtils.writeFile(outputs, cacheFilePath, true);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void loadCache() {
		...
		if (new File(cacheFilePath).exists())
			lines = FileUtils.readFile(cacheFilePath);
	...
	public List<String> execute(String sparql) // sparql select * where { <fb:g.11vjx745d> ?y ?z}
	{
	...
	public List<String> executeFuzzyQuery(String sp) {
		...
		try {
			...
			for (String triple : this
					.execute("sparql select ?x where {<" + subj.replace("ns:", "http://rdf.freebase.com/ns/") + "> <"
							+ pred.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?x .}")) {
				String mid = triple;
				result.add(mid);
			}
			if (result.size() == 0) {
				List<String> triples = this
						.execute("sparql select ?x ?z where {<" + subj.replace("ns:", "http://rdf.freebase.com/ns/")
								+ "> ?x ?y." + "?y <" + pred.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?z .}");
				if (triples.size() > 60000)
					return names;
				String newPred = new String();
				for (String triple : triples) {
					String mid = triple.split("\t")[1];
					newPred = triple.split("\t")[0].replace("http://rdf.freebase.com/ns/", "ns:") + ".."
							+ pred.replace("ns:", "");
					result.add(mid);
				}
				names.add(newPred);
	...
	public Set<String> retrieveNeighbourNodes(String sp) {
		...
		List<String> triples = this
				.execute("sparql select ?z where {<" + subj.replace("ns:", "http://rdf.freebase.com/ns/") + "> <"
						+ firstPred.replace("ns:", "http://rdf.freebase.com/ns/") + ">?x." + "?x ?y ?z .}");
	...
	public Set<String> executePOQuery(String po) {
		...
		try {
			...
			for (String triple : this
					.execute("sparql select * where {?x <" + pred.replace("ns:", "http://rdf.freebase.com/ns/") + "> <"
	...
	public Set<String> retrieveAnswerType(String sp) throws Exception {
		...
		if (pred.indexOf("..") != -1) {
			...
			for (String triple : this
					.execute("sparql select ?x where {<" + subj.replace("ns:", "http://rdf.freebase.com/ns/") + "> <"
							+ firstPredicate.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?x. " + "?x <"
							+ secondPredicate.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?y .}"
	...
	public Set<String> retrieveRelations(String subj) throws Exception {
		...
		for (String triple : this.execute("sparql select ?x ?z where {<"
				+ subj.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?x ?y. " + "?y ?z ?m .}")) {
	...
	public List<String> retrieveWikiIds(String mid) throws Exception {
		List<String> triples = this.execute("sparql select ?x where { <http://rdf.freebase.com/en/" + mid
				+ "> <http://rdf.freebase.com/ns/type.namespace.keys> ?x}");
	...
	public List<String> retrieveNames(String mid) {
		List<String> triples = this.execute("sparql select ?x where { <http://rdf.freebase.com/ns/" + mid
				+ "> <http://rdf.freebase.com/ns/type.object.name> ?x}");
		return triples;
	}

	public String retrieveDescription(String mid) {
		List<String> triples = this.execute("sparql select ?x where { <http://rdf.freebase.com/ns/" + mid
				+ "> <http://rdf.freebase.com/ns/common.topic.description> ?x}");
		if (triples.size() == 0)
			return "";
	...
	public Set<String> execute2HopQuery(String subj, String firstPred, String secondPred, String sibling) {
		...
		List<String> triples = this
				.execute("sparql select ?z ?target where {<" + subj.replace("ns:", "http://rdf.freebase.com/ns/")
						+ "> <" + firstPred.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?x. " + "?x <"
						+ secondPred.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?z." + "?x ?n ?target .}");
		...
		if (results.size() == 0) {
			triples = this
					.execute("sparql select ?z ?target where {<" + subj.replace("ns:", "http://rdf.freebase.com/ns/")
							+ "> <" + firstPred.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?x." + "?x <"
							+ secondPred.replace("ns:", "http://rdf.freebase.com/ns/") + "> ?z. " + "?x ?n ?y. "

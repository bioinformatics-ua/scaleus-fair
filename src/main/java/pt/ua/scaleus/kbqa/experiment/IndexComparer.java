...
import pt.ua.scaleus.kbqa.index.DBOIndex;
import pt.ua.scaleus.kbqa.index.Patty_relations;
...
public class IndexComparer {
	...
	public static void main(final String[] args) {
		List<KBQAQuestion> questions = getQALDQuestions();
		List<List<HashSet<String>>> questionsWithSetsToCompare = new ArrayList<>();
		for (KBQAQuestion x : questions) {
	 ...
	 */
	private static List<KBQAQuestion> getQALDQuestions() {
		...
		try {
			questions = KBQAQuestionFactory.createInstances(QALD_Loader.loadJSON(url.openStream()));
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(0);
		}
	 ...
	 * Data.
	 */
	private static HashSet<String> getSparql(final KBQAQuestion q) {
	 ...
	 * obtained Entities]
	 */
	private static List<HashSet<String>> getSets(final KBQAQuestion q) {
